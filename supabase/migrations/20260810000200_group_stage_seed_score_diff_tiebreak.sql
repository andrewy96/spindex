-- advance_beylive_round's group_stage pool-standings ranking (used to pick
-- the top 2 of each pool for the Top 16 bracket) only tiebroke tied win
-- counts by total points scored, with no score-difference tiebreak at all —
-- unlike the "Pool standings" UI (beyliveGroupPools in src/lib/beylive.ts),
-- which tiebreaks wins by score difference (points - against) before total
-- points. That mismatch meant the player the UI displayed as advancing
-- (ranked #1/#2, highlighted) could differ from who the database actually
-- seeded into the knockout bracket whenever two players in a pool tied on
-- wins but had different opponents' scores.
--
-- Fix: compute each standing's `against` (points conceded) alongside
-- `points`, and rank pools by wins desc, (points - against) desc, points
-- desc, user_id — matching the front-end's tiebreak order exactly.
