-- Supabase runs API/RPC connections (the "authenticated" role) with
-- pg_safeupdate active, which rejects any UPDATE/DELETE with no WHERE
-- clause — including on temp tables. assign_group_stage_pools and
-- advance_beylive_round clear their scratch temp tables with a bare
-- `delete from ...`, which pg_safeupdate blocks with "DELETE requires a
-- WHERE clause" the moment a host tries to draw pools. Adding `where true`
-- keeps the exact same behavior (delete everything) while satisfying the
-- guard.
...
begin
  ...
  delete from _gs_pool_state where true;
  ...
  delete from _gs_pool_clubs where true;
  ...
  delete from _gs_players where true;
