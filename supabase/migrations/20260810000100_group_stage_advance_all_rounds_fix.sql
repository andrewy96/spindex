-- group_stage tournaments pre-create every pool round (rounds 1..N) up front
-- (see create_beylive_group_stage_matches), instead of generating them one
-- round at a time like other formats. advance_beylive_round's group_stage
-- branch didn't account for this: it only checked whether pool matches
-- existed for round_no = cur + 1 (they always do, since every round was
-- pre-created) and, if so, just bumped tournament.current_round by one and
-- returned — without generating the Top 16 knockout bracket. So a host had
-- to click "Advance / Finalize" once per pool round (5 times for a 5-round
-- group stage) before the bracket-seeding code at the bottom of the branch
-- was ever reached, and the first several clicks produced no visible change,
-- making it look like the bracket simply never generated even after every
-- match showed "all matches done".
--
-- Fix: check whether ANY pool match anywhere is still incomplete, not just
-- whether round_no = next_round has rows. Once the whole group stage is
-- actually finished, a single "Advance / Finalize" click seeds the Top 16
-- bracket directly, regardless of how far current_round had been walked
-- forward already.
