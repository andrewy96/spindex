-- Close registration bypasses:
-- - payment proof is always required for submitted registrations
-- - team name is required whenever the team-name field is enabled
-- - old direct join_tournament cannot skip the registration form

update public.tournaments
set registration_config =
  jsonb_set(
    jsonb_set(
      coalesce(registration_config, '{}'::jsonb),
    ...
    '{teamNameRequired}',
    case
      when coalesce((registration_config->>'teamNameEnabled')::boolean, true)
        then 'true'::jsonb
      else 'false'::jsonb
    end,
    true
...
begin
  ...
  if coalesce((cfg->>'enabled')::boolean, false) is true then
  ...
  if joined_count >= t.max_players then
...
begin
  ...
  select payment_proof_path into existing_proof_path
  from public.tournament_registrations
  where tournament_id = tid and user_id = auth.uid();

  if proof_path is null and nullif(trim(coalesce(existing_proof_path, '')), '') is null then
