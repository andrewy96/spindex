-- Host registration settings must be customizable:
-- team name can be enabled but optional, and custom question required flags are authoritative.
...
begin
  ...
  team_enabled := coalesce((cfg->>'teamNameEnabled')::boolean, true);
  team_required := team_enabled and coalesce((cfg->>'teamNameRequired')::boolean, team_enabled);
  if team_required is true and nullif(trim(coalesce(new.team_name, '')), '') is null then
    raise exception 'team_name_required';
  end if;
