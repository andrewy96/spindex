...
export default function SharePodiumModal({
  data,
  fileId,
  onClose,
}: {
  ...
  useEffect(() => {
    ...
    setFile(null);
    (async () => {
      await Promise.all([ensureFonts(podiumSampleText(card)), ensureLogoImage()]);
      if (cancelled) return;
  ...
  const share = async () => {
    ...
    try {
      await navigator.share({
        files: [file],
        title: "SPINDEX",
        text: `${card.tournamentName} podium`,
      });
    } catch (err) {
      if ((err as DOMException)?.name === "AbortError") return;
      download();
      showToast("Couldn't open the share sheet — saved instead.");
    }
  };
    ...
    >
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          <h3 className="font-display text-sm font-bold tracking-wider">Share podium</h3>
          <button onClick={onClose} className="text-xs text-ink-dim transition hover:text-ink">
            ✕ Close
          </button>
        </div>

        <div className="relative overflow-hidden rounded-lg border border-edge">
          <canvas ref={canvasRef} width={CARD_W} height={CARD_H} className="block h-auto w-full" />
          {!file && (
        ...
        {toast && <p className="mt-2 text-center text-xs font-semibold text-accent">{toast}</p>}
      </div>
    </div>
  );
}
